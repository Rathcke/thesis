...
\frame{\frametitle{The query}
  ...
  \begin{itemize}
    ...
    \item If same piece, check if this distance is smaller. For $Q(u,\lambda)$, total
      query time is $O(\text{size}(\lambda)\cdot r+n/\sqrt{r}\lg^2)$.
  \end{itemize}
  \centering
...
\frame{\frametitle{Updates}
  ...
  \begin{itemize}
    \item Edge weight changes straightforward.
...
\frame{\frametitle{The oracle}
  ...
  The oracle is built on top of an $r$-division, where we store additively weighted
  Voronoi diagrams to allow point to point location in $O(\lg n)$ time. For labels that
...
\frame{\frametitle{The oracle: Analysis}
  ...
  to the point location structure taking $O(\sqrt{r})$ time. Since we do this at most $O(\text{polylog}(n))$
...
\frame{\frametitle{The oracle: Analysis}
  ...
  can construct these in $O(r)$ time and extend them in $O(\sqrt{r})$ time to support
  point location. There are $O(n)$ of these, for a total of $O(nr)$. Total time spent is
  $\tilde{O}(n^{3/2})$. \\
  $ $ \\
