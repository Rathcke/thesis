...
\subsection{The planar case}\label{surveyplanar}
...
with $S\in [n^{4/3}, n^{3/2}]$, he gives a distance oracle with query time $O(n\lg
  n/\sqrt{S})$. \\
  This trade-off, $\Theta(n/\sqrt{S})$, was later improved to work over the entire range
  ...
  \cite{cabello2006many}.  Mozes and Sommer \cite{mozes2012exact} obtained the same
  result, but with an improvement for $S\in [n, n\lg \lg n$ and for any constant
    $\varepsilon > 0$. They manage to produce a distance oracle with preprocessing time
    $O(n\lg n)$, space $O(S)$ and a query time of $O(n^{1/2+\varepsilon})$. This result
    was simultaneously found by Nussbaum \cite{nussbaum2011improved}.


\subsection{Distance oracle in a planar graph with $O(n^{3/2})$ space and $O(\lg n)$ query
time.}\label{cohenplanar}
